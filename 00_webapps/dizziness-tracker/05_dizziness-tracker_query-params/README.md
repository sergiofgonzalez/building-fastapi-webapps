...
> Step 5: Adding URL query parameters

In this step we add URL query parameters. The app is also enhanced a little bit to start playing the Episode schema concept, and the Pydantic schema definitions are simplified as `Field` definitions are not needed (for now, those will be needed when we need to do things such as put a limit on the length of the data sent, etc.)
